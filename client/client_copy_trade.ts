...
async function runCopyTradeClient() {
    console.log(`=============================================================`);
    console.log(` Executing /copy-intelligence Client (Payer: ${walletB.address})`);
    ...
    console.log('\n[Client] Requesting /copy-intelligence feed with x402 payment...');
    try {
        const response = await fetchWithPayment('http://localhost:3000/copy-intelligence', {
            method: 'POST',
        ...
        console.log('\n=============================================================');
        console.log(' [Client] Copy Trading Intelligence Feed Received:');
        console.log('=============================================================');
        console.log(` Recommended Pool ID:  ${data?.recommendedPool?.id}`);
        console.log(` Pool Pair:           ${data?.recommendedPool?.token0?.symbol} / ${data?.recommendedPool?.token1?.symbol} (Fee Tier: ${data?.recommendedPool?.feeTier})`);
        console.log(` Current Pool Tick:   ${data?.recommendedPool?.tick}`);
        console.log(` Working Tick Width:  ${data?.workingTickWidth} ticks`);
        console.log(` Suggested Tick Range: [${data?.suggestedRange?.tickLower}, ${data?.suggestedRange?.tickUpper}] (Aligned Width: ${data?.suggestedRange?.alignedTickWidth})`);
        console.log(` Top LPs Analyzed:    ${data?.topWalletsAnalyzed?.length || 0} wallets`);
        console.log(` Total LP Positions:  ${data?.totalPositionsAnalyzed || 0} positions`);
        ...
        if (Array.isArray(data?.preparedTransactions) && data.preparedTransactions.length > 0) {
            ...
            console.log(`\n=============================================================`);
            console.log(` [Client] All Copy Trade Transactions Successfully Executed!`);
            console.log(`=============================================================`);
        }
    } catch (err: any) {
